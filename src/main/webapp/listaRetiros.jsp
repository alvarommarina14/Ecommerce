...
<%@page import="Entities.RetiroSucursal"%>
...
<% 
    LinkedList<RetiroSucursal> lista = (LinkedList<RetiroSucursal>) request.getAttribute("lista");
...
<section class="py-5 text-center container" id="ct-table" style="background-image: none;height: auto;">
    <div class="row py-lg-5">
      <div class="col-lg-6 col-md-8 mx-auto">
        <h5>Estos son los retiros que todavia estan pendientes</h5>
        <p>
  ...
  <tbody>
  <%for(RetiroSucursal rs: lista){ %>
    <tr>
      <th scope="row"><%=rs.getPedido().getId() %></th>
      <td><%= rs.getCliente().getNroDocumento()%></td>
      <td><%= rs.getCliente().getNombreApellido()%></td>
      <td><%= rs.getPedido().getFecha()%></td>
      <td><%= rs.getFechaRetiroEstimada() %></td>
      <td><%= rs.getCliente().getDireccion()%></td>
      <td><%= rs.getCosto()%></td>
    </tr> 
    <% } %>
  </tbody>
